# Política de Privacidade - NEØ.FLOWOFF
...
### 2.3 Informações de Blockchain

- **Endereços de Wallet**: Armazenados localmente no seu dispositivo via localStorage
- **Transações**: Dados públicos da blockchain (quando você interage com o token $NEOFLW)
- **Saldos**: Informações de saldo do token (consultadas via RPC público)
...
- Processar e gerenciar sua conta e wallet
- Facilitar transações e interações com o token $NEOFLW
...
- **Wallet State**: Armazenado localmente no seu dispositivo via `localStorage`
- **Preferências**: Configurações e preferências do usuário armazenadas localmente
- **Nenhuma chave privada é armazenada**: Suas chaves privadas permanecem sob seu controle

### 4.2 Armazenamento de Servidor

- Informações de contato e formulários podem ser armazenadas em nossos servidores
- Dados são mantidos apenas pelo tempo necessário para os fins descritos nesta política
...
- **Provedores de Pagamento**: Para processar transações (quando aplicável)

Esses prestadores de serviços são obrigados a manter a confidencialidade de suas informações.
...
- Em conexão com uma investigação de atividade ilegal
...
- **Email**: privacidade@flowoff.com.br
