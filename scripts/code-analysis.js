...
class PWACodeAnalyzer {
  ...
  analyzeFile(filePath) {
    try {
      ...
      // Contar por tipo
      this.stats.files.byType[fileType] =
        (this.stats.files.byType[fileType] || 0) + 1;
  ...
  analyzeHTML(content, filePath, lineCount) {
    ...
    if (content.includes('<meta name="viewport"'))
      this.stats.html.withViewport++;
    if (content.includes('<meta name="theme-color"'))
      this.stats.html.withThemeColor++;
    if (content.includes('<link rel="manifest"'))
      this.stats.html.withManifest++;
    ...
    const semanticTags = (
      content.match(/<(main|nav|header|footer|section|article|aside)/g) || []
    ).length;
    ...
    if (!content.includes('manifest.webmanifest')) {
      this.stats.issues.pwa.push({
        ...
        suggestion:
          'Adicione <link rel="manifest" href="manifest.webmanifest">',
      });
    }
    ...
    const hasServiceWorkerScript = content.match(
      /<script[^>]*src\s*=\s*["'][^"']*(app\.js|index-scripts\.js)/i
    );
  ...
  analyzeCSS(content, filePath, lineCount) {
    ...
    if (content.includes('env(safe-area-inset'))
      this.stats.css.pwaSpecific.safeArea++;
    if (content.includes('backdrop-filter'))
      this.stats.css.pwaSpecific.backdropFilter++;
    if (content.includes('-webkit-'))
      this.stats.css.pwaSpecific.webkitPrefixes++;
  ...
  analyzeServiceWorker(content, filePath) {
    ...
    if (content.includes('cache-first'))
      this.stats.serviceWorker.cacheStrategy = 'cache-first';
    else if (content.includes('network-first'))
      this.stats.serviceWorker.cacheStrategy = 'network-first';
    else if (content.includes('stale-while-revalidate'))
      this.stats.serviceWorker.cacheStrategy = 'stale-while-revalidate';
    ...
    if (content.includes('skipWaiting'))
      this.stats.serviceWorker.updateStrategy = 'skipWaiting';
    else if (content.includes('clients.claim'))
      this.stats.serviceWorker.updateStrategy = 'clients.claim';
  }
  ...
  reportFiles() {
    ...
    this.log(
      `Total de arquivos analisados: ${this.stats.files.total}`,
      'white'
    );
    ...
    this.log('\nPor tamanho:', 'yellow');
    this.log(
      `  Pequenos (<50 linhas): ${this.stats.files.bySize.small}`,
      'green'
    );
    this.log(
      `  Médios (50-200 linhas): ${this.stats.files.bySize.medium}`,
      'yellow'
    );
    this.log(
      `  Grandes (200-500 linhas): ${this.stats.files.bySize.large}`,
      'orange'
    );
    this.log(
      `  Muito grandes (>500 linhas): ${this.stats.files.bySize.xlarge}`,
      'red'
    );

    this.log('\nArquivos PWA essenciais:', 'yellow');
    this.log(
      `  Presentes: ${this.stats.files.pwa.total}/${CONFIG.pwaFiles.length}`,
      'green'
    );
  ...
  reportHTML() {
    ...
    this.log(
      `Com service worker: ${this.stats.html.withServiceWorker}`,
      'cyan'
    );
    ...
    this.log(
      `  Aria labels: ${this.stats.html.accessibility.ariaLabels}`,
      'cyan'
    );
    this.log(
      `  Tags semânticas: ${this.stats.html.accessibility.semanticTags}`,
      'cyan'
    );
  }
  ...
  reportCSS() {
    ...
    this.log(
      `  Backdrop filter: ${this.stats.css.pwaSpecific.backdropFilter}`,
      'cyan'
    );
    this.log(
      `  Webkit prefixes: ${this.stats.css.pwaSpecific.webkitPrefixes}`,
      'cyan'
    );
  }
  ...
  reportJavaScript() {
    ...
    this.log(
      `  Notifications: ${this.stats.js.pwaFeatures.notifications}`,
      'cyan'
    );
  }
  ...
  reportPWA() {
    ...
    this.log(
      `  Existe: ${this.stats.manifest.exists ? '✓' : '✗'}`,
      this.stats.manifest.exists ? 'green' : 'red'
    );
    this.log(
      `  Válido: ${this.stats.manifest.valid ? '✓' : '✗'}`,
      this.stats.manifest.valid ? 'green' : 'red'
    );
    ...
    this.log(
      `  Existe: ${this.stats.serviceWorker.exists ? '✓' : '✗'}`,
      this.stats.serviceWorker.exists ? 'green' : 'red'
    );
    this.log(
      `  Cache strategy: ${this.stats.serviceWorker.cacheStrategy}`,
      'cyan'
    );
    this.log(
      `  Offline support: ${this.stats.serviceWorker.offlineSupport ? '✓' : '✗'}`,
      this.stats.serviceWorker.offlineSupport ? 'green' : 'red'
    );
    this.log(
      `  Update strategy: ${this.stats.serviceWorker.updateStrategy}`,
      'cyan'
    );
  }
  ...
  reportPerformance() {
    ...
    this.log(
      `  Otimizadas: ${this.stats.performance.images.optimized}`,
      'cyan'
    );
    this.log(`  WebP: ${this.stats.performance.images.webp}`, 'cyan');

    const optimizationRate =
      this.stats.performance.images.total > 0
        ? (
            (this.stats.performance.images.optimized /
              this.stats.performance.images.total) *
            100
  ...
  reportIssues() {
    ...
    if (this.stats.issues.performance.length > 0) {
      this.log(
        `\nPerformance (${this.stats.issues.performance.length}):`,
        'yellow'
      );
    ...
    if (this.stats.issues.accessibility.length > 0) {
      this.log(
        `\nAcessibilidade (${this.stats.issues.accessibility.length}):`,
        'yellow'
      );
    ...
    if (this.stats.issues.structure.length > 0) {
      this.log(
        `\nEstrutura (${this.stats.issues.structure.length}):`,
        'yellow'
      );
  ...
  reportRecommendations() {
    ...
    if (!this.stats.manifest.exists) {
      recommendations.push(
        'Crie um manifest.webmanifest para funcionalidade PWA completa'
      );
    }

    if (!this.stats.serviceWorker.exists) {
      recommendations.push(
        'Implemente um service worker para cache e funcionalidade offline'
      );
    }
    ...
    if (this.stats.html.withThemeColor === 0) {
      recommendations.push(
        'Adicione meta theme-color para personalização do browser'
      );
    }
    ...
    if (
      this.stats.performance.images.optimized <
      this.stats.performance.images.total * 0.8
    ) {
    ...
    if (
      this.stats.html.accessibility.altTags === 0 &&
      this.stats.performance.images.total > 0
    ) {
    ...
    if (this.stats.html.accessibility.semanticTags < 3) {
      recommendations.push(
        'Use mais tags semânticas (main, nav, header, footer)'
      );
    }

    if (recommendations.length === 0) {
      this.log(
        '🎉 Excelente! Sua PWA está seguindo as melhores práticas.',
        'green'
      );
    } else {
