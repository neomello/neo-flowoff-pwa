...
async function validateToken() {
  ...
  try {
    ...
    log(
      `✅ Chain ID: ${config.network.chainId} (${config.network.name})`,
      'green'
    );
    ...
    if (chainId !== config.network.chainId) {
      log(
        `⚠️  Chain ID do RPC (${chainId}) diferente do configurado (${config.network.chainId})`,
        'yellow'
      );
    } else {
...
async function validateLayout() {
  ...
  try {
    ...
    if (swInHtml || swExists) {
      ...
      log(
        `⚠️  Service Worker não encontrado (verifique sw.js e registro)`,
        'yellow'
      );
    }
...
async function validateWalletIntegration() {
  ...
  try {
    ...
    if (
      walletCode.includes('polygon-rpc.com') ||
      walletCode.includes('TOKEN_CONFIG.network')
    ) {
...
async function main() {
  ...
  log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`, 'cyan');
  log(
    `  ${passed}/${total} validações passaram`,
