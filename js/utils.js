...
function createSafeElement(tag, attributes = {}, textContent = '') {
  ...
  const allowedAttributes = ['id', 'class', 'type', 'href', 'src', 'alt', 'title', 'aria-label', 'role', 'data-route'];
  ...
  Array.from(doc.body.childNodes).forEach(node => {
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      Array.from(node.attributes).forEach(attr => {
        if (attr.name.startsWith('on') || attr.name === 'javascript:' || attr.name === 'data:' && !attr.value.startsWith('data:image/')) {
